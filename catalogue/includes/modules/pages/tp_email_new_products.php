...
*/
  ...
  class tp_email_new_products {
    var $group = 'email_new_products';
    var $template = '';
    var $title = 'New Products Template';
    var $description = 'osCommerce Basic New Products Template Node with welcome text';
    var $section = 'undefined';
    var $version = '1.01';

    function prepare() {
      global $oscTemplate, $nInfo, $emsinfo, $mimemessage;
      $oscTemplate->_email_data['newsletters']['enable_osc_mail'] = 'True';
    }

    function build() {
      global $oscTemplate, $nInfo, $mimemessage, $HTTP_GET_VARS;
	
	  if ($oscTemplate->_email_data['newsletters']['enable_osc_mail'] == 'True') {
		
		$name = sprintf(TEXT_WELCOME, $nInfo->mail['firstname']);
		$email_address = $nInfo->mail['email'];	
		$anniversary_date = $nInfo->mail['anniversary_date'];	
						
		echo "We are in the template file.";
		$products = array();
		session_start();
		$products = $_SESSION['product_id'];
		//echo '<pre>';
		//	var_dump($products);
		//	echo '</pre>';
		//	die();
        ...
        $mimemessage->send($nInfo->mail['firstname'] . ' ' . $nInfo->mail['lastname'], $nInfo->mail['email'], '', EMAIL_FROM, $nInfo->title);
