  ...
  class authorizenet_cc_aim {
    ...
    function selection() {
      return array('id' => $this->code,
                   'module' => tep_image(DIR_WS_ICONS . 'generic-card.png') . ' ' . $this->public_title);
    }
    ...
    function after_process() {
	  ...
	  // Hack by Dan to include card type in order status
      $status[] = 'Card Code: ' . tep_db_prepare_input($cvv2_response) . $response['x_card_type'];
...
EOD;
      ...
      if ( MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_SERVER == 'Live' ) {
        $info .= 'Live Server:<br />https://secure2.authorize.net/gateway/transact.dll';
      } else {
    ...
    function getTestConnectionResult() {
      if ( MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_SERVER == 'Live' ) {
        $api_url = 'https://secure2.authorize.net/gateway/transact.dll';
      } else {
