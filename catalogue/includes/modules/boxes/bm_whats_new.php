...
*/
  ...
  class bm_whats_new {
    ...
    function execute() {
      global $currencies, $oscTemplate, $request_type;

      if ($random_product = tep_random_select("select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_status = '1' and products_site = '" . STORE_SITE . "' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW)) {
        ...
        $data =  '<div class="panel panel-default">' .
                 ...
                 '  <div class="panel-body text-center"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . tep_image(DIR_WS_PRODUCT_IMAGES . substr($random_product['products_image'],2), $random_product['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br /><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $random_product['products_name'] . '</a><br />' . $whats_new_price . $request_type . '</div>' .
                 '</div>';
