        ...
        if ($product_check['total'] > 0) {
          ...
          if ( tep_db_num_rows($product_info_query) === 1 ) {
            ...
            if ( tep_not_null(MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_APP_ID) ) {
			  ...
			  $data['fb:admins'] = MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_APP_ID;
            }

            $product_description = substr(trim(preg_replace('/\s\s+/', ' ', strip_tags($product_info['short_desc']))), 0, 197);
            ...
            $data['og:image'] = DIR_WS_PRODUCT_IMAGES . substr($products_image,2);
