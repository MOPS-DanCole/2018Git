...
*/
  ...
  class tp_email_EMS_MKTG20 {
    var $group = 'email_EMS_MKTG20';
    var $template = '';
    ...
    function build() {
	  ...
	  if ($oscTemplate->_email_data['newsletters']['enable_osc_mail'] == 'True') {
		...
		$name = sprintf(TEXT_WELCOME, $nInfo->mail['firstname'] . ' ' . $nInfo->mail['lastname']);
		$email_address = $nInfo->mail['email'];	
				
		//echo "Now in tp_email_EMS_MKTG20.php";
    ...
    function preview() {
      ...
      if (tep_db_num_rows($newsletter_query)) {
        ...
        //$name = sprintf(TEXT_WELCOME, $mail['firstname'] . ' ' . $mail['lastname']);
		//$name = TEXT_WELCOME . $mail['firstname'] . ' ' . $mail['lastname'];
		
		//echo "<br><br>" . $newsletter['module'];
		//echo "<br><br>" . $name;
		//die();
