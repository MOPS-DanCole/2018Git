...
*/
  ...
  $expected_query = tep_db_query("select p.products_id, pd.products_name, products_date_available as date_expected from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where to_days(products_date_available) >= to_days(now()) and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' AND (p.products_category = '" . STORE_CATEGORY . "' OR p.products_category = '" . STORE_CATEGORY2 . "' OR p.products_category = '" . STORE_CATEGORY3 . "' OR p.products_category = '" . STORE_CATEGORY4 . "' OR p.products_category = '" . STORE_CATEGORY5 . "') AND (p.products_site = 'ALL' OR p.products_site = '" . STORE_SITE . "') order by " . EXPECTED_PRODUCTS_FIELD . " " . EXPECTED_PRODUCTS_SORT . " limit " . MAX_DISPLAY_UPCOMING_PRODUCTS);
